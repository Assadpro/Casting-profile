...
<section class="voting-section">
  ...
  <div id="nomineesGrid" class="nominees-grid" aria-live="polite">
    <!-- server-render initial nominees for SEO and initial paint -->
...
<!-- subscribe modal (required before vote) -->
...
// --- Replace broken click delegation and missing handlers with robust logic ---
// attach vote button handlers (delegation) - expects vote buttons to have data-vote-id="<id>"
document.addEventListener('click', function(e){
  const btn = e.target.closest('.vote-button, [data-id]');
  if (!btn) return;
  e.preventDefault();

  // find nominee id from common attributes or parent card
  const nidAttr = btn.getAttribute('data-vote-id') || btn.getAttribute('data-id') || (btn.closest('.nominee-card') && btn.closest('.nominee-card').getAttribute('data-id'));
  const nid = parseInt(nidAttr || '0', 10);
  if (!nid) return;

  // require subscription token
  if (!subscriberToken) {
    pendingNomineeId = nid;
    if (typeof openSubscribe === 'function') openSubscribe(nid);
    return;
  }

  doVote(nid, subscriberToken, btn);
});
...
document.getElementById('doSubscribe').addEventListener('click', async function(){
  ...
  try {
    ...
    try { data = JSON.parse(text); } catch (err) {
      if (msgEl) msgEl.textContent = 'Server error: ' + (text ? text.slice(0,300) : res.status + ' ' + res.statusText);
      return;
    }
    if (!res.ok || !data.success) {
      if (msgEl) msgEl.textContent = data.error || 'Subscription failed';
      return;
    }
    // success: store token and proceed to vote if pending
    subscriberToken = data.token;
    localStorage.setItem('sf_sub_token', subscriberToken);
    if (msgEl) msgEl.textContent = 'Subscribed. Casting your vote...';
    closeSubscribe();
    if (pendingNomineeId) {
      // small delay to allow modal close UI to update
      const nid = pendingNomineeId;
      pendingNomineeId = 0;
      await doVote(nid, subscriberToken);
    }
  } catch (err) {
    if (msgEl) msgEl.textContent = 'Network error: ' + (err && err.message ? err.message : 'unknown');
  } finally {
    btn.disabled = false;
  }
});
...
// voting action
async function doVote(nomineeId, token, btnEl = null) {
  if (!nomineeId) return;
  if (!token) {
    pendingNomineeId = nomineeId;
    openSubscribe(nomineeId);
    return;
  }
  ...
  try {
    ...
    let data;
    try { data = JSON.parse(text); } catch (err) {
      alert('Server error: ' + (text ? text.slice(0,300) : res.status + ' ' + res.statusText));
      return;
    }

    if (!res.ok) {
      // if subscriber missing -> prompt subscribe modal
      const errMsg = (data && data.error) ? data.error.toString().toLowerCase() : '';
      if (errMsg.includes('subscriber not found') || errMsg.includes('subscribe')) {
        pendingNomineeId = nomineeId;
        openSubscribe(nomineeId);
        return;
      }
    ...
    if (data.success) {
      ...
      totalVotes = (typeof totalVotes === 'number') ? totalVotes + 1 : totalVotes;
      const floatEl = document.getElementById('floatTotal');
      if (floatEl) floatEl.textContent = totalVotes;
      // store share token + nominee name for the share modal
      window._sf_share_token = data.share_token || '';
      window._sf_shared_name = (document.querySelector('.nominee-card[data-id="'+nomineeId+'"] .nominee-name') || {}).textContent || '';
      if (data.message) alert(data.message);
      if (typeof openShare === 'function' && data.share_token) {
        openShare();
      }
    } else {
      // common server-side rejection (already voted etc.)
...
// small helper to open subscribe modal
function openSubscribe(nid){
  pendingNomineeId = nid || pendingNomineeId;
  ...
  const msg = document.getElementById('subMsg'); if (msg) msg.textContent = '';
}
...
// share modal helpers
function openShare(){
  const token = window._sf_share_token || '';
  const name = window._sf_shared_name || ( (pendingNomineeId && document.querySelector('.nominee-card[data-id=\"'+pendingNomineeId+'\"] .nominee-name')) ? document.querySelector('.nominee-card[data-id=\"'+pendingNomineeId+'\"] .nominee-name').textContent : 'the nominee' );
  const m = document.getElementById('shareModal');
  const actorEl = document.getElementById('sharedActor');
  const linkEl = document.getElementById('shareLinkText');
  if (actorEl) actorEl.textContent = name;
  const shareUrl = (location.origin || (location.protocol + '//' + location.host)) + '/vote_for_award.php?shared=' + encodeURIComponent(token);
  ...
  if (tw) tw.href = 'https://twitter.com/intent/tweet?text=' + encodeURIComponent('I just voted for '+name+' — '+shareUrl);
  if (wa) wa.href = 'https://api.whatsapp.com/send?text=' + encodeURIComponent('I just voted for '+name+' — '+shareUrl);
  if (m) m.style.display = 'block';
}
function closeShare(){ const m = document.getElementById('shareModal'); if (m) m.style.display = 'none'; }
...
// notify backend when user shares (calls /ajax/share_callback.php)
...
async function pollStandings(){
  try {
    ...
    // update nominee cards and leaderboard
    const nominees = data.nominees || [];
    // update nominee cards
